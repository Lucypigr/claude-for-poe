...
- `src/items/`：物品資料層（無 DOM）。`ITEM_DEFINITIONS` 為凍結的共用基礎資料；`ItemInstance` 以唯一 `uid` 識別並只以 `defId` 參照定義，個別狀態放在 `data`。`Inventory` 是唯一的格子背包模型，所有放置／移動都回傳明確驗證結果，失敗不改變狀態；UI 只能透過它改變物品位置。
